...
bool UDPSocket::init(bool ipv6, bool noExceptions)
{
	...
	dstream << "UDPSOCKET::INIT " << std::endl<<std::endl;
...
void UDPSocket::Bind(Address addr)
{
	...
	if(m_addr_family == AF_INET6) {
		...
		if(bind(m_handle, (const struct sockaddr *) &address,
			...
			dstream << (int) m_handle << ": Bind failed for AF_INET6: "
					<< "   address:  " << addr.serializeString()
					<< "   port:   " << htons(addr.getPort()) << "   strerror:   "
...
int UDPSocket::Receive(Address & sender, void *data, int size)  //18082017
{
	// Return on timeout
	...
	//if (socket_enable_debug_output || sender.serializeString() == "128.173.21.152")  {
	if (socket_enable_debug_output ) {
