...
void KeyCache::populate()
{
	...
	key[KEYMAP_ID_GENDER]     = getKeySetting("keymap_gender");
	key[KEYMAP_ID_MIRROR]       = getKeySetting("keymap_mirror");
	key[KEYMAP_ID_KINECT_TOGGLE]    = getKeySetting("keymap_kinect_toggle");
	...
	key[KEYMAP_ID_UPPERHALF]
			= getKeySetting("keymap_upperhalf");
		...
		bool *connect_ok, bool *aborted)
{
	...
	*aborted = false;
	...
	//if (connect_address_kinect.getPort() != " 52504")
		//Address connect_address_kinect(0, 0, 0, 0, 52504);	
	//if (connect_address_kinect.getPort() != " 52505")
	...
	int convertedpdataport = stoi(pdataport);
		...
		Address connect_address_kinect(0, 0, 0, 0, convertedkinectport);
	client->connectKinect(connect_address_kinect, *address,
		!(simple_singleplayer_mode || local_server_mode));

		Address connect_address_pdata(0, 0, 0, 0, convertedpdataport);
	client->connectPdata(connect_address_pdata, *address,
		!(simple_singleplayer_mode || local_server_mode));
		...
		try {
			...
			dstream << "client->Receive() " << std::endl;
			client->Receive();   //18082017
			dstream << "client->ReceiveKinect() " << std::endl;
			//client->ReceiveKinect();     //18082017
			dstream << "client->ReceivePdata() " << std::endl;
			//client->ReceivePdata();

		} catch (con::NoIncomingDataException &e) {
		} catch (con::PeerNotFoundException &e) {
			infostream<<"Client <=====> Kinect: PeerNotFoundException"<<std::endl;
		} 
...
void Game::updatePlayerControl(const CameraOrientation &cam)
{
	...
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	player->keyPressed = keypress_bits;   //18082017

	//tt.stop();

	if (input->wasKeyDown(keycache.key[KeyCache::KEYMAP_ID_MIRROR])) {
	...
	if (input->wasKeyDown(keycache.key[KeyCache::KEYMAP_ID_UPPERHALF])) {
		if (player->upperhalf == false)
			player->upperhalf = true;
		else
			player->upperhalf = false;
	}
	
	if (input->wasKeyDown(keycache.key[KeyCache::KEYMAP_ID_GENDER])) {
	...
	if (input->wasKeyDown(keycache.key[KeyCache::KEYMAP_ID_KINECT_TOGGLE])) {
		if (player->kinecttoggle == false)
			player->kinecttoggle = true;
		else
			player->kinecttoggle = false;
	}
}
