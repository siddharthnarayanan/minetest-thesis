...
// set_bone_position_quaternion(bone, position, (w)rotation, (x)axis.x, (y)axis.y , (z)axis.z)
int ObjectRef::l_set_bone_position_quaternion(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	ServerActiveObject *co = getobject(ref);
	...
	if (!lua_isnil(L, 4))
		angle = luaL_checknumber(L, 4) * core::DEGTORAD;
	if (!lua_isnil(L, 5))
		orthoX = luaL_checknumber(L, 5);
	if (!lua_isnil(L, 6))
		orthoZ = luaL_checknumber(L, 6);

	v3f tmp_rotation;

	if (bone != "Head")
	{
		v3f rotation_vec = v3f(orthoX, 0.0F, orthoZ);
		core::quaternion rotation;
		rotation_vec.normalize();
		rotation.fromAngleAxis(angle, rotation_vec);
		rotation.toEuler(tmp_rotation);
		tmp_rotation *= core::RADTODEG;
		co->setBonePosition(bone, position, tmp_rotation);
	}
	...
	if (bone == "Head")
	{
		...
		//angle		------------>	//PITCH
		orthoX *= core::DEGTORAD;  	//YAW
		orthoZ *= core::DEGTORAD;	//ROLL

		rotationheadX.fromAngleAxis(angle, axisX);
		rotationheadY.fromAngleAxis(orthoX, axisY);
		rotationheadZ.fromAngleAxis(orthoZ, axisZ);
		...
		rotation_head.toEuler(tmp_rotation);
		tmp_rotation *= core::RADTODEG;
		co->setBonePosition(bone, position, tmp_rotation);
	}
...
// get_kinect_pitch(self)
int ObjectRef::l_get_kinect_pitch(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_pitch  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectPitch());
	return 1;
}

// get_kinect_yaw(self)
int ObjectRef::l_get_kinect_yaw(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_yaw  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectYaw());
	return 1;
}

// get_kinect_roll(self)
int ObjectRef::l_get_kinect_roll(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_roll  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRoll());
	return 1;
}

// get_kinect_arm_l(self)
int ObjectRef::l_get_kinect_arm_l(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_l  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftArm());
	return 1;
}

// get_kinect_shoulder_l(self)
int ObjectRef::l_get_kinect_shoulder_l(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_shoulder_l  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftShoulder());
	return 1;
}

//get_kinect_arm_l_OrthoX(self)
int ObjectRef::l_get_kinect_arm_l_OrthoX(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_l_OrthoX  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftArmOrthoX());
	return 1;
}

//get_kinect_arm_l_OrthoZ(self)
int ObjectRef::l_get_kinect_arm_l_OrthoZ(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_l_OrthoZ  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftArmOrthoZ());
	return 1;
}

// get_kinect_arm_r(self)
int ObjectRef::l_get_kinect_arm_r(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_r  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightArm());
	return 1;
}

// get_kinect_shoulder_r(self)
int ObjectRef::l_get_kinect_shoulder_r(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_shoulder_r  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightShoulder());
	return 1;
}

//get_kinect_arm_r_OrthoX(self)
int ObjectRef::l_get_kinect_arm_r_OrthoX(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_r_OrthoX  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightArmOrthoX());
	return 1;
}

//get_kinect_arm_r_OrthoZ(self)
int ObjectRef::l_get_kinect_arm_r_OrthoZ(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_arm_r_OrthoZ  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightArmOrthoZ());
	return 1;
}

//get_kinect_leg_l(self)
int ObjectRef::l_get_kinect_leg_l(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_l  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftLeg());
	return 1;
}

//get_kinect_leg_l_OrthoX(self)
int ObjectRef::l_get_kinect_leg_l_OrthoX(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_l_OrthoX  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftLegOrthoX());
	return 1;
}

//get_kinect_leg_l_OrthoZ(self)
int ObjectRef::l_get_kinect_leg_l_OrthoZ(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_l_OrthoZ  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftLegOrthoZ());
	return 1;
}

// get_kinect_leg_r(self)
int ObjectRef::l_get_kinect_leg_r(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_r  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightLeg());
	return 1;
}

//get_kinect_leg_r_OrthoX(self)
int ObjectRef::l_get_kinect_leg_r_OrthoX(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_r_OrthoX  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightLegOrthoX());
	return 1;
}

//get_kinect_leg_r_OrthoZ(self)
int ObjectRef::l_get_kinect_leg_r_OrthoZ(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_leg_r_OrthoZ  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightLegOrthoZ());
	return 1;
}

//get_kinect_torso_rot(self)
int ObjectRef::l_get_kinect_torso_rot(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_torso_rot  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectTorsoRot());
	return 1;
}

//get_kinect_torso_roll(self)
int ObjectRef::l_get_kinect_torso_roll(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_torso_roll  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectTorsoRoll());
	return 1;
}

//get_kinect_torso_x(self)
int ObjectRef::l_get_kinect_torso_x(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_torso_x  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectTorsoX());
	return 1;
}

//get_kinect_left_shoulder_flag(self)
int ObjectRef::l_get_kinect_left_shoulder_flag(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_left_shoulder_flag  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectLeftShoulderFlag());
	return 1;
}

//get_kinect_right_shoulder_flag(self)
int ObjectRef::l_get_kinect_right_shoulder_flag(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_right_shoulder_flag  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectRightShoulderFlag());
	return 1;
}

//get_pdata_face(self)
int ObjectRef::l_get_pdata_face(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_pdata_face  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getPureDataFace());
	return 1;
}

//get_gender(self)
int ObjectRef::l_get_gender(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_gender  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getGender());
	return 1;
}

//get_kinect_face(self)
int ObjectRef::l_get_kinect_face(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	Player *player = getplayer(ref);
	if (player == NULL) return 0;
	// Do it
	verbosestream <<   "  ObjectRef::l_get_kinect_face  called for:  "    << player->getName() << std::endl;
	lua_pushnumber(L, player->getKinectFace());
	return 1;
}
...
const luaL_reg ObjectRef::methods[] = {
	...
	luamethod(ObjectRef, set_bone_position_quaternion),
	...
	luamethod(ObjectRef, get_kinect_pitch),
	luamethod(ObjectRef, get_kinect_yaw),
	luamethod(ObjectRef, get_kinect_roll),
	luamethod(ObjectRef, get_kinect_arm_l),
	luamethod(ObjectRef, get_kinect_shoulder_l),
	luamethod(ObjectRef, get_kinect_arm_l_OrthoX),
	luamethod(ObjectRef, get_kinect_arm_l_OrthoZ),
	luamethod(ObjectRef, get_kinect_arm_r),
	luamethod(ObjectRef, get_kinect_shoulder_r),
	luamethod(ObjectRef, get_kinect_arm_r_OrthoX),
	luamethod(ObjectRef, get_kinect_arm_r_OrthoZ),
	luamethod(ObjectRef, get_kinect_leg_l),
	luamethod(ObjectRef, get_kinect_leg_l_OrthoX),
	luamethod(ObjectRef, get_kinect_leg_l_OrthoZ),
	luamethod(ObjectRef, get_kinect_leg_r),
	luamethod(ObjectRef, get_kinect_leg_r_OrthoX),
	luamethod(ObjectRef, get_kinect_leg_r_OrthoZ),
	luamethod(ObjectRef, get_kinect_torso_rot),
	luamethod(ObjectRef, get_kinect_torso_roll),
	luamethod(ObjectRef, get_kinect_torso_x),
	luamethod(ObjectRef, get_kinect_left_shoulder_flag),
	luamethod(ObjectRef, get_kinect_right_shoulder_flag),
	luamethod(ObjectRef, get_kinect_face),
	luamethod(ObjectRef, get_pdata_face),
	luamethod(ObjectRef, set_look_yaw),
	luamethod(ObjectRef, set_look_pitch),
	luamethod(ObjectRef, get_gender),
